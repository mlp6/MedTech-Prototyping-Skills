...
\frame{
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item Documentation: \url{https://docs.kicad.org/7.0/en/getting_started_in_kicad/getting_started_in_kicad.html}
    \end{itemize}
}
...
\frame{
  ...
  \centerline{\includegraphics[width=0.5\textwidth]{commands_overview.png}}

  \begin{center}
    \begin{footnotesize}
      \url{https://docs.kicad.org/7.0/en/eeschema/eeschema.html\#schematic-editing-operations}
    \end{footnotesize}
  \end{center}
}
...
\frame{
 ...
 \begin{frame}
  \centerline{\includegraphics[width=0.7\textwidth]{schematic_annotated.png}}
 \end{frame}

 \subsection{In-Class Exercise}

 \begin{frame}{In-Class Exercise: Part I}
Create a simple LED powered by a battery.
 \end{frame}

 \begin{frame}
    \frametitle{Schematic Components Not on the PCB}
    
    If a component will not exist on the PCB:
    \begin{itemize}
        \item Be sure to exclude it from the board (\texttt{Properties}
        $\rightarrow$ \texttt{General} $\rightarrow$ \textbf{Exclude from Board}
        / \textbf{Do Not Place}).  
        \item Represent the \textbf{Connectors} or \textbf{Terminal Blocks} on
        th schematic that will be used to connect the component to the PCB by
        wires.
    \end{itemize}
 \end{frame}

 \begin{frame}{In-Class Exercise: Part II}
 
    Modify your circuit to:
    \begin{itemize}
        \item Specify using a CR2032 coin cell battery that will be mounted on the PCB.
        \item Connect the LED off the PCB using a connector.
    \end{itemize}
\end{frame}
 
\begin{frame}{Heirarchecal Sheets}

    \begin{itemize}
        \item Circuits can get very complicated very quickly; quickly divorced
        from functional signal flow.
        \item Can organize functional sub-circuits into ``sheets'' (similar to
        writing functions in software development).
        \item Sheets require explicit types of ``pins'' to allow signals to communicate between sheets.
        \item Power ports are ``global'' and do not need to be explicitly connects.
    \end{itemize}
...
\begin{frame}
  ...
  \centerline{\includegraphics[width=0.6\textwidth]{065-0100_Main_Board-1.png}}
\end{frame}

\begin{frame}
  \frametitle{A current project...}

  \centerline{\includegraphics[width=0.6\textwidth]{065-0100_Main_Board-5.png}}
\end{frame}

\begin{frame}
  \frametitle{A current project...}

  \centerline{\includegraphics[width=0.6\textwidth]{065-0100_Main_Board-6.png}}
\end{frame}

\begin{frame}
  \frametitle{A current project...}

  \centerline{\includegraphics[width=0.6\textwidth]{065-0100_Main_Board-3.png}}
\end{frame}

 \begin{frame}{Bill of Materials (BOM)}
    \begin{itemize}
        \item All of the components in the schematic can be populated into a Bill of Materials.
        \item Industry-standard ECAD packages will be automatically linked to online part catalogs to order.
        \item Specific components can be excluded from the BOM (\texttt{Properties}).
    \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{frame}
    ...
    \begin{columns}
      ...
      \end{column}
      \begin{column}{0.5\textwidth}
        \centerline{\includegraphics[width=1.0\textwidth]{pcb_layers_annotated.png}}
      \end{column}
    \end{columns}
  ...
  \begin{frame}
    ...
    \centerline{\includegraphics[width=0.8\textwidth]{pcbnew_ui.png}}
  \end{frame}
  ...
  \begin{frame}
      ...
      \centerline{\includegraphics[width=0.75\textwidth]{thermal_relief.jpg}}
  ...
  \frame{
    ...
    \begin{columns}
    \begin{column}{0.5\textwidth}
      \centerline{\includegraphics[width=1.0\textwidth]{grounds01.png}}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centerline{\includegraphics[width=1.0\textwidth]{grounds02.png}}
    \end{column}
  \end{columns}
...
\begin{itemize}
    ...
    \item \href{https://docs.kicad.org/7.0/en/pcbnew/pcbnew.html}{KiCad Docs: PCB Editor}
    \item \href{https://gitlab.oit.duke.edu/EmbeddedMedicalDevices/kicad_test}{Example KiCad Project}
